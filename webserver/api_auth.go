...
func (a *apiAuth) login(w http.ResponseWriter, r *http.Request) {
	...
	// get user info
	userInfo, err := a.service.GetUserInfoByUsername(authClaim.Username)
	if err == nil {
		authClaim.Avatar = userInfo.Avatar
		authClaim.Bio = userInfo.Bio
		authClaim.FullName = userInfo.FullName
	}
...
func (a *apiAuth) register(w http.ResponseWriter, r *http.Request) {
	...
	userInfo := storage.UserInfo{
		...
		FullName:  authClaim.FullName,
		CreatedAt: time.Now(),
	}
	err = a.db.Create(&userInfo)
	if err != nil {
		utils.Response(w, http.StatusInternalServerError, err, nil)
		return
	}
