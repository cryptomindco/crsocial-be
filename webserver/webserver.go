...
func NewWebServer(c Config, db storage.Storage, mailClient *email.MailClient) (*WebServer, error) {
	...
	if c.SiteRoot == "" {
		return nil, fmt.Errorf("please set up siteRoot")
	}
...
func (s *WebServer) loggedInMiddleware(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		localAuthClaims := authClaims{
			...
			Role:     utils.UserRole(exClaims.Role),
			Expire:   exClaims.Expire,
			UserName: exClaims.Username,
		}

		// get userinfo from DB
		userInfo, err := s.service.GetUserInfoByUsername(exClaims.Username)
		if err != nil {
			utils.Response(w, http.StatusBadRequest, &utils.InternalError, nil)
			return
		}
		localAuthClaims.FullName = userInfo.FullName
		localAuthClaims.Email = userInfo.Email
		localAuthClaims.Avatar = userInfo.Avatar
		localAuthClaims.Bio = userInfo.Bio
...
func (s *WebServer) getLoginInfoMiddleware(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		var bearer = r.Header.Get("Authorization")
		exClaims, isLogin := s.checkMicroServiceLoginMiddleware(r, bearer)
		if !isLogin {
			ctx := context.WithValue(r.Context(), authClaimsCtxKey, &authClaims{
				IsLogin: false,
			})
			next.ServeHTTP(w, r.WithContext(ctx))
			return
...
func (s *WebServer) adminMiddleware(next http.Handler) http.Handler {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		if claims.Role != utils.UserRoleAdmin {
			e := &utils.Error{
